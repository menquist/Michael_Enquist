...
#file_name = "Chatbot_COT-data.xlsx"
file_name = "Chatbot-dat.xlsx"
lines = pd.read_excel(file_name, sheet_name="documents")
#pd_dict = {k: g["line"].tolist() for k,g in lines.groupby("QA")}
#pd_dict = lines.line
...
seq = cycle([1, 2])
lines['Seq'] = [next(seq) for count in range(lines.shape[0])]
lines
...
#pd_dict = pd_dict[pd_dict['QA']==1]


# In[8]:


#x = pd_dict.values.tolist()
#def split_list(a_list):
#    half = len(a_list)//2
#    return a_list[:half], a_list[half:]
#questions, answers  = split_list(x)
#y=pd_dict.iloc[:, 4]
#questions
...
questions = lines[lines['Seq']==1]
questions = questions['Text'].values.tolist()
answers = lines[lines['Seq']==2]
answers = answers['Text'].values.tolist()
answers
...
checkpoint = "chatbot_weights.ckpt.meta"
session = tf.InteractiveSession()
session.run(tf.global_variables_initializer())
saver = tf.train.Saver()
