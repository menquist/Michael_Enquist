...
# \itemize{
#   \item `"stock.prices"`: Get the open, high, low, close, volume and adjuste
...
#'   \item [quandl_api_key("c58SRnkLsYYrxJs6d74e")] to set the api key for collecting data via the `"quandl"`
#'   get option.
#  av_api_key("ZLX1IQYC2DN82YK7") # to set the api key for collecting data via the `"alphavantage"`
#'   get option.
#}
#'
#'
...
tq_get <- function(x, get = "stock.prices", complete_cases = TRUE, ...) {
  ...
  if("quandl" %in% get) {
    if (is.null(quandl_api_key("c58SRnkLsYYrxJs6d74e"))) warning("No Quandl API key detected. Limited to 50 anonymous calls per day. Set key with 'quandl_api_key()'.",
                                           call. = FALSE)
  }
  
  # Validate Alpha Vantage api key
  if("alphavantager" %in% get) {
    if (is.null(av_api_key("ZLX1IQYC2DN82YK7"))) stop("No Alpha Vantager API key detected. Set key with 'av_api_key()'.",
                                               call. = FALSE)
  }
